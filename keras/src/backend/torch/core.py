...
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    device = get_device()  # cache at function start
    ...
    # Handle list/tuple of torch tensors by stack (special fast path)
    if isinstance(x, (list, tuple)):
        if len(x) > 0 and all(is_tensor(x1) for x1 in x):
            # stack all tensors together in one call
            return torch.stack(x)
        elif len(x) > 0 and any(is_tensor(x1) for x1 in x):
            # recursively convert and stack
            return torch.stack([convert_to_tensor(x1) for x1 in x])

    # Convert array-like non-list/tuple to np.ndarray only if needed
    if not isinstance(x, (list, tuple)):
        x = np.array(x, copy=False)

    # Handle np.ndarray-specific conversions
    if isinstance(x, np.ndarray):
        if x.dtype == np.uint32:
            # Torch backend does not support uint32.
            x = x.astype(np.int64)
        # Special bfloat16 handling
        dt = standardize_dtype(x.dtype)
        if dt == "bfloat16":
