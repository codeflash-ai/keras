...
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    if dtype is None:
        if isinstance(x, bool):
            # Avoid repeated get_device calls
            return torch.as_tensor(x, dtype=torch.bool, device=device)
        elif isinstance(x, int):
            return torch.as_tensor(x, dtype=torch.int32, device=device)
        elif isinstance(x, float):
            # Avoid repeated floatx call
            fx = floatx()
            return torch.as_tensor(x, dtype=to_torch_dtype(fx), device=device)
    # Amortize: if already np.ndarray, skip additional conversion
    if not isinstance(x, (list, tuple, np.ndarray, torch.Tensor)):
        x = np.array(x)
    elif isinstance(x, (list, tuple)):
        if x and any(isinstance(x1, torch.Tensor) for x1 in x):
            # Handle list or tuple of torch tensors, re-use device
            return torch.stack([convert_to_tensor(x1) for x1 in x])
    # After here, x is np.ndarray, or list/tuple with no Tensors
    ...
    if dtype is None:
        # Reuse flatten result so we only walk it once
        flat_x = tree.flatten(x)
        # Pre-resolve getattr lookups
        dtypes = [getattr(item, "dtype", type(item)) for item in flat_x]
        dtype = result_type(*dtypes)
