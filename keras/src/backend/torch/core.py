...
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    if isinstance(x, Variable) or is_tensor(x):
        ...
        if dtype is not None:
            # Optimize: avoid redundant to_torch_dtype if already torch dtype
            to_dtype = dtype
            if not (
                isinstance(dtype, torch.dtype)
                or (
                    hasattr(dtype, "__module__")
                    and dtype.__module__.startswith("torch")
                )
            ):
                to_dtype = to_torch_dtype(dtype)
            x = x.to(to_dtype)
        return x
    if dtype is None:
    ...
    if not isinstance(x, (list, tuple)):
        ...
        # Only go to stack if there's at least one tensor, postponing np.array creation
    ...
    # Handle NumPy array case for dtype logic
    base_x = np_x
    if isinstance(np_x, np.ndarray):
        if np_x.dtype == np.uint32:
            # Torch backend does not support uint32.
            np_x = np_x.astype(np.int64)
        if standardize_dtype(np_x.dtype) == "bfloat16":
            # Torch backend does not support converting bfloat16 ndarray.
            np_x = np_x.astype(np.float32)
            dtype = "bfloat16"
        dtype = dtype or np_x.dtype

    if dtype is None:
        # Optimize flatten for np.ndarray (use .flat or .ravel() which is much faster for contiguous blocks)
        if is_np:
            # np_x is an np.ndarray here
            items = np_x.flat
            dtype = result_type(
                *[
                    item.dtype if hasattr(item, "dtype") else type(item)
                    for item in items
                ]
            )
        elif isinstance(np_x, (list, tuple)):
            # Fast path for "flat" lists or tuples, otherwise fallback
            # If the elements themselves are sequences, flatten with tree
            any_seq = any(
                isinstance(el, (list, tuple, np.ndarray, torch.Tensor))
                for el in np_x
            )
            if any_seq:
                dtype = result_type(
                    *[
                        ...
                        for item in tree.flatten(np_x)
                    ]
                )
            else:
                dtype = result_type(
                    *[getattr(item, "dtype", type(item)) for item in np_x]
                )
        else:
            # fallback
            dtype = result_type(
    ...
    return torch.as_tensor(np_x, dtype=dtype, device=get_device())
