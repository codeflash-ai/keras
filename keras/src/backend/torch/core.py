...
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    # Cache device used for consistency and perf
    device = get_device()

    # Fastpath: x is Variable or torch Tensor
    ...
    # At this point, x is not Tensor/Variable/Python primitive
    # Handle list/tuple with tensors efficiently
    if isinstance(x, (list, tuple)):
        x_list = x
        x_len = len(x_list)
        if x_len > 0:
            # Manual short-circuit Tensor membership test (hot path)
            for x1 in x_list:
                if isinstance(x1, torch.Tensor):
                    # Avoid list comprehension to reduce allocations for large x
                    return torch.stack([convert_to_tensor(x1) for x1 in x_list])
        # Fallback to numpy array (next section)
    if not isinstance(x, np.ndarray):
        x = np.array(x)

    # At this point, x is a numpy ndarray
    ...
    if dtype is None:
        # Flatten only if necessary (i.e., x is not already flat array)
        # For most ndarrays, iterating gives scalar or subarrays/objects, so always flatten
        to_flat = tree.flatten(x)
        # get types for result_type
        types_for_result = [
            getattr(item, "dtype", type(item)) for item in to_flat
        ]
        dtype = result_type(*types_for_result)
