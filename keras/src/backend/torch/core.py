...
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    device = get_device()
    # Handle Variable and torch.Tensor types efficiently
    ...
    if not isinstance(x, (list, tuple)):
        ...
        # x is list or tuple
        if len(x) > 0 and any(is_tensor(x1) for x1 in x):
            # List/tuple contains tensor(s): stack all after conversion
            return torch.stack([convert_to_tensor(x1) for x1 in x])
        if isinstance(
            x, np.ndarray
        ):  # unlikely due to logic, but kept for safety
            arr = x
        else:
            arr = np.array(x)
    # Handle numpy array specifics
    if isinstance(arr, np.ndarray):
        if arr.dtype == np.uint32:
            arr = arr.astype(np.int64)
        standardized = standardize_dtype(arr.dtype)
        if standardized == "bfloat16":
            arr = arr.astype(np.float32)
            dtype = "bfloat16"
        dtype = dtype or arr.dtype
    # For dtype inference, only flatten if we still don't know dtype
    if dtype is None:
        # Only flatten if dtype is still unknown
        flat_dtypes = [
