...
def get_device():
    ...
    if device is None:
        _default_device_cache = DEFAULT_DEVICE
...
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    to_torch_dtype_ = to_torch_dtype  # faster local lookup
    is_tensor_ = is_tensor
    device = get_device()
    # Fast path for Variable or Tensor objects
    if isinstance(x, Variable) or is_tensor_(x):
    ...
    if dtype is None:
        if isinstance(x, bool):
            ...
            cached_dtype = to_torch_dtype_(floatx())
            return torch.as_tensor(x, dtype=cached_dtype, device=device)
    # List/tuple containing tensors: avoid repeated calls
    if isinstance(x, (list, tuple)):
        has_tensor = False
        if x:
            # To avoid generator double iteration
            tensor_flags = tuple(is_tensor_(x1) for x1 in x)
            if any(tensor_flags):
                return torch.stack([convert_to_tensor(x1) for x1 in x])
    else:
        # Convert to NumPy if not list/tuple
        x = np.array(x)
    if isinstance(x, np.ndarray):
        if x.dtype == np.uint32:
            # Torch backend does not support uint32.
            x = x.astype(np.int64)
        standardized = standardize_dtype(x.dtype)
    ...
    if dtype is None:
        # Only call tree.flatten ONCE for better memory perf
        flat_items = tree.flatten(x)
        dtype = result_type(
            *(getattr(item, "dtype", type(item)) for item in flat_items)
        )
    dtype = to_torch_dtype_(dtype)
...
def cast(x, dtype):
    is_tensor_ = is_tensor
    to_torch_dtype_ = to_torch_dtype
    dtype = to_torch_dtype_(dtype)
    if isinstance(x, Variable):
        x = x.value
    if is_tensor_(x):
