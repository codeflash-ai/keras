...
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    if isinstance(x, Variable) or is_tensor(x):
        device = get_device()
        tensor = x.value if isinstance(x, Variable) else x
        # Avoid unnecessary .to() if already on device
    ...
    device = get_device()
    # If dtype is None and x is a Python scalar, optimize conversion path.
    if dtype is None:
    ...
    # Fast-path: list/tuple of tensors, use stack
    if isinstance(x, (list, tuple)):
        if x and all(is_tensor(x1) for x1 in x):
            # Use generator expression for memory efficiency
            return torch.stack(tuple(convert_to_tensor(x1) for x1 in x))
        # Check if non-tensor array-likes inside list/tuple need flattening
    # Convert to np array if not already ndarray, list, or tuple.
