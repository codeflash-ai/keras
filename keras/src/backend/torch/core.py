...
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    device = get_device()  # Get once for all hot paths

    # Handle python scalars early for performance
    if dtype is None:
    ...
    # If it's not a list or tuple, convert to np.array unless already ndarray
    ...
    # If dtype not yet set, infer result type from flattened values
    if dtype is None:
        flat = tree.flatten(x)
        flat_types = [getattr(item, "dtype", type(item)) for item in flat]
