...
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    # Local device for this call, avoid repeated global lookups
    device = get_device()

    # Fast path if already a Variable or Tensor
    if isinstance(x, Variable) or is_tensor(x):
        if isinstance(x, Variable):
            x = x.value
        x_device = x.device if hasattr(x, "device") else None
        if x_device != device:
    ...
    if not isinstance(x, (list, tuple)):
        ...
        # Short-circuit for empty list/tuple: dtype fallback and avoid flatten logic
        result_dtype = to_torch_dtype(
            dtype if dtype is not None else result_type()
        )
        x_arr = np.array(x)
        return torch.as_tensor(x_arr, dtype=result_dtype, device=device)
    elif any(is_tensor(x1) for x1 in x):
        # Handle list or tuple of torch tensors, preserve device/dtype
    ...
    if isinstance(x_arr, np.ndarray):
        arr = x_arr
        arr_dtype = arr.dtype
        # Special case: Torch backend does not support uint32
        if arr_dtype == np.uint32:
            arr = arr.astype(np.int64)
            arr_dtype = arr.dtype
        # Special case: Torch backend does not support converting bfloat16 ndarray.
        s_dtype = standardize_dtype(arr_dtype)
        if s_dtype == "bfloat16":
            arr = arr.astype(np.float32)
            arr_dtype = arr.dtype
            dtype = "bfloat16"
        # Use dtype precedence: passed param -> arr -> fallback
        final_dtype = dtype or arr_dtype
    else:
        arr = x_arr
        final_dtype = dtype or (getattr(x_arr, "dtype", type(x_arr)))

    if final_dtype is None:
        # Only flatten if really necessary (no dtype given yet)
        flat_values = list(tree.flatten(arr))
        resolved_types = [
            getattr(item, "dtype", type(item)) for item in flat_values
        ]
        inferred_dtype = result_type(*resolved_types)
        final_dtype = inferred_dtype

    torch_dtype = to_torch_dtype(final_dtype)
