...
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    # Standardize dtype once if needed, and reuse the value.
    _dtype = standardize_dtype(dtype) if dtype is not None else None

    # Fast path: already array and dtype matches.
    if isinstance(x, (jnp.ndarray, jax.Array)):
        if _dtype is None or x.dtype == _dtype:
            return x
    ...
    if isinstance(x, Variable):
        x_value = x.value
        if _dtype is not None and x_value.dtype != _dtype:
            return x_value.astype(_dtype)
        return x_value
    ...
    if isinstance(x, jax_sparse.JAXSparse):
        if sparse is not None and not sparse:
            x = x.todense()
        elif _dtype is not None and x.dtype != _dtype:
            return x.astype(_dtype)
        else:
            return x

    # Inline the 'is_tensor' logic (see keras/src/backend/jax/core.py):
    is_jax_tensor = isinstance(x, (jnp.ndarray, jax_sparse.JAXSparse))
    if not is_jax_tensor and _dtype == "bfloat16":
        # Can't create bfloat16 arrays directly from non-array objects
        return jnp.asarray(x).astype(_dtype)
