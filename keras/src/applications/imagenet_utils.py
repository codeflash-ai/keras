...
def decode_predictions(preds, top=5):
    ...
    # Efficient lookup outside loop
    class_index = CLASS_INDEX

    preds = ops.convert_to_numpy(preds)
    results = []
    for pred in preds:
        # Use argpartition to get indices of top K
        if top < pred.shape[0]:
            top_indices = np.argpartition(pred, -top)[-top:]
            # Now sort the top indices by prediction score descending
            top_indices = top_indices[np.argsort(pred[top_indices])[::-1]]
        else:
            # If top >= 1000, fallback to full sort
            top_indices = np.argsort(pred)[::-1]

        # Precompute string keys for lookup to avoid repeated str() calls
        str_indices = [str(i) for i in top_indices]
        # Build tuple (class_name, description, score)
        result = [
            tuple(class_index[idx_str]) + (pred[idx],)
            for idx_str, idx in zip(str_indices, top_indices)
        ]
        # No need to sort explicitly; already sorted
