...
def _validate_input_shape_channels(
    ...
    if input_shape is not None:
        ...
        # Localize input_shape[axis] to avoid redundant attribute lookup
        shape_channel = input_shape[axis]
        if shape_channel != 3 and weights == "imagenet":
            raise ValueError(
                "The input must have 3 channels; Received "
                f"`input_shape={input_shape}`"
            )

        # Precompute axes instead of repeatedly producing the list
        if axis == 0:
            h_idx, w_idx = 1, 2
        elif axis == -1:
            h_idx, w_idx = 0, 1
        else:
            # Fallback for safety, keep original logic
            axes = [i for i in range(3) if i != axis]
            h_idx, w_idx = axes[0], axes[1]
        h_dim = input_shape[h_idx]
        w_dim = input_shape[w_idx]

        # Combine height/width checks for early exit
        if (
            (h_dim is not None and h_dim < min_size)
            or (w_dim is not None and w_dim < min_size)
        ):
            raise ValueError(
